//THERE IS 2 SOLUTION THE 2nd ONE IS THE PERFECT SOLUTION
...
stDate IncreaseDateByXDays(stDate DateFrom, short X)
{
...
int main()
{
    ...
    DateFrom = IncreaseDateByXDays(DateFrom, VacationDays);
   
    cout << endl << endl;

    cout << "Return Date : " << DayShortName(CalculateDayOfWeekOrder(DateFrom.Day, DateFrom.Month, DateFrom.Year));
...
stDate CalculateVacationReturnDate(stDate DateFrom, short VacationDays)
{
    short WeekCounter = 0;
     ...
     //in Case Data is Weekend Keep Adding one until is Business Day
    while (IsWeekend(DateFrom))
    {
        DateFrom = IncreaseDateByOneDay(DateFrom);
    }

    //Increasing The Vacation Dates to add all Weekends to it
    for (short i = 1; i <= VacationDays + WeekCounter; i++)
    {
     ...
     //in Case The Return Date is Weekend keep Adding one day until is business day
    while (IsWeekend(DateFrom))
    {
        DateFrom = IncreaseDateByOneDay(DateFrom);
    }
